...
namespace KinaUnaXamarin.Views
{
    ...
    public partial class VaccinationsPage : ContentPage
    {
        private int _viewChild = Constants.DefaultChildId;
        private UserInfo _userInfo;
        private VaccinationsViewModel _viewModel;
        ...
        private async Task UpdateVaccinations()
        {
            _viewModel.IsBusy = true;
            
            List<Vaccination> vaccinations =
                await ProgenyService.GetVaccinationsList(_viewChild, _viewModel.UserAccessLevel);
            
            if (vaccinations != null && vaccinations.Count > 1)
            {
                vaccinations = vaccinations.OrderBy(v => v.VaccinationDate).ToList();
                _viewModel.VaccinationItems.ReplaceRange(vaccinations);
                
                VaccinationsListView.ScrollTo(0);
            }
            _viewModel.IsBusy = false;
        }
